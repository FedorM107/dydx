...
def open_positions(client):
    ...
    # Initialize container for BotAgent resutls
    ...
    for index, row in df.iterrows():
        ...
        if len(series_1) > 0 and len(series_2) == len(series_2):
            ...
            if abs(z_score) >= ZSCORE_THRESH:
                ...
                #Ensure like-for_like not already open (diversify trading)
                ...
                if not is_base_open and not is_quote_open:
                    ...
                    #Adjustment for Pairs that must adhere to quantity step of 10 
                    for particolari in TOKEN_FACTOR_10:
                        if base_market == particolari:
                            base_quantity = base_quantity - base_quantity % 10
                        if quote_market == particolari:
                            quote_quantity = quote_quantity - quote_quantity % 10
                    ...
                    #Insure size (Consider max order size)
